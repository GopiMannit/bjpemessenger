  ...
  //   const [data, setData] = useState(JSON.parse(sessionStorage.getItem('data')));
    ...
    const handlefromdateChange = (event) => {
      ...
      if(todate !== "")
      {
      axios.get(`${SERVER_URL}api/report/${event.target.value}/${todate}`)  
      .then(response => {
      ...
      if (data.length !== 0) {
        ...
        const tableData = data.map(item => {
          return {
            ...
            Date: new Date(item.dateCreated).toLocaleDateString('en-IN')
          };
        });
    ...
    const handletodatechange=(event)=>{
      ...
      axios.get(`${SERVER_URL}api/report/${fromdate}/${event.target.value}`)  
      .then(response => {
          setData(response.data);
          // console.log(dataString);
          // sessionStorage.setItem('data', dataString);
    ...
    const columns = useMemo(
      () => [
        ...
        },
        {
          ...
          //or in the component override callbacks like this
          Cell: ({ cell }) => {
            return <div className="cursor-pointer underline text-[#fb660d]" onClick={ handlePreviewClick1}>View</div>;
          },
        },
        {
          accessorKey: 'videoid',
          ...
          //or in the component override callbacks like this
          Cell: ({ cell }) => {
            return <div className="cursor-pointer underline text-[#fb660d]" onClick={handlePreviewClick}>View</div>;
          },
        },{
      ...
      <div  className='mt-12'>
          ...
          <div className="flex justify-between w-full">
              ...
              <input 
                  ...
                  className="w-[240px] bg-white border shadow-[0px_0px_3px] shadow-[#fb660d] focus:outline-none"
  
              />
              </div>
              <div className="flex flex-col space-y-1">
              <label htmlFor="videoUrl" className='font-semibold text-[17px] mb-2 text-[#00A650]'>
                    To Date
              </label>
                <input 
                  type="date"
                ...
                <div className='mt-6'>
                <button
              className="text-white w-[100px] p-2 rounded-sm btncolor border shadow-[0px_0px_5px] shadow-[#fb660d] "
              onClick={handleClear}
            >
              Clear
            </button>
                ...
                <div className='mt-6'>
                <button
              className="text-white p-2 rounded-sm btncolor border shadow-[0px_0px_5px] shadow-[#fb660d]"
            ...
            >
              ...
              initialState={{ pagination: { pageSize: 5, pageIndex: 2 } }}
